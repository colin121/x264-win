...
#define SET_LEVEL(ndst, nsrc, l) {\
	...
	{\
		...
		trellis_level_t tmp = { nsrc.level_idx, l };\
        level_tree[levels_used] = tmp;\
	}\
                         ...
                         int b_chroma, int dc, int num_coefs, int idx )
{
    ...
    cabac_state = &h->cabac.state[ coeff_abs_level_m1_offset[ctx_block_cat] ];
                         ...
                         int b_chroma, int dc, int num_coefs, int idx, int b_8x8 )
{
    ...
    int i, j;
	int last_nnz, coef_mask, round_mask;
    const int f = 1<<15;
